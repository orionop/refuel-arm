#!/usr/bin/env python
"""
NEO controller as a ROS1 (Noetic) node for UR5 + Gazebo.
...
Publishes:
    /<controller>/command  (trajectory_msgs/JointTrajectory)
        default controller name: "ur5_arm_controller"
...
Assumptions:
    ...
    - Packages available on the lab PC:
    ...
    - UR5 model loaded via `roboticstoolbox.models.UR5()`
    - Controller takes JointTrajectory commands (default JointTrajectoryController)
      The node integrates NEO's joint velocities once and sends the next joint
      target as a single-point trajectory.

Usage:
    rosrun <pkg> neo_ros_node.py
    or:
...
CONTROLLER_TOPIC_DEFAULT = "/ur5_arm_controller/command"
JOINT_NAMES_DEFAULT = [
    "shoulder_pan_joint",
    "shoulder_lift_joint",
    "elbow_joint",
...
TARGET_DEFAULT = [0.4, -0.4, 0.4]  # x, y, z in base frame
CONTROL_RATE_DEFAULT = 100.0       # Hz
INFLUENCE_DIST_DEFAULT = 0.30      # m
STOP_DIST_DEFAULT = 0.05           # m
OBSTACLE_RADIUS_DEFAULT = 0.05     # m, assumed sphere; can be replaced per obs
...
class NEONode:
    def __init__(self):
        ...
        # Initial joint guess; will be replaced when first /joint_states arrives
        self.q = np.array([0.0, -np.pi / 2, np.pi / 2, -np.pi / 2, -np.pi / 2, 0.0])
        self.have_joint_state = False
        self.joint_lock = threading.Lock()

        # Obstacles: dict name -> sg.Sphere with .pose and .v in world frame
        self.obstacles = {}
        ...
        rospy.loginfo(
            "[neo_controller] init complete. publishing to %s",
    ...
    def joint_state_cb(self, msg: JointState):
        ...
        try:
            ...
            return  # expected names not present yet (e.g. gripper-only update)
        with self.joint_lock:
            self.q = q_new
            self.have_joint_state = True

    def model_states_cb(self, msg: ModelStates):
        """Pick up obstacles by name prefix; record pose and (estimated) velocity."""
        ...
        with self.obs_lock:
            for i, name in enumerate(msg.name):
                ...
                if name not in self.obstacles:
                    ...
                    # Update pose on the existing sphere
                    sph.T = pose.A  # spatialgeometry expects 4x4
                # NEO uses .v as the obstacle linear+angular velocity (6-vec)
    ...
    def neo_step(self):
        """Solve one NEO QP step. Returns qd (n-vec) or None if infeasible."""
        with self.joint_lock:
        ...
        Q = np.eye(n + 6)
        Q[:n, :n] *= Y
        # Slack weighting (1/e diverges near goal; clamp)
        ...
        for collision in obstacles_snapshot:
            try:
                c_Ain, c_bin = robot.link_collision_damper(
                    ...
                    self.ds,
                    1.0,
                    start=robot.link_dict["shoulder_link"],
                    end=robot.link_dict["tool0"],
                )
            except Exception:
                continue
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # Integrate joint velocity to next joint target
            with self.joint_lock:
                q_now = self.q.copy()
            q_next = q_now + qd * dt
