...
# ── IK-Geo import (Localized) ────────────────────────────────────
from ik_geometric import rot, fwd_kinematics, KIN_KR6_R700, KIN_UR5 # type: ignore


# ═══════════════════════════════════════════════════════════════════
#  Kinematics Constants (KUKA KR6 R700)
# ═══════════════════════════════════════════════════════════════════

KIN = KIN_KR6_R700
H_AXES = KIN['H']
P_VECS = KIN['P']
# rot is already imported
...
def fk_segment_endpoints(q):
    ...
    Returns: [base, shoulder, elbow, wrist1, wrist2, ee_flange, nozzle_tip]
    """
    ...
    # Add Nozzle Tip
    NOZZLE_LENGTH = 0.15
    # KUKA Tool axis is X; UR5 is Y
    tool_axis = R[:, 0] if np.array_equal(KIN['H'], KIN_KR6_R700['H']) else R[:, 1]
...
def _dist_segment_to_box(p1: np.ndarray, p2: np.ndarray, center: np.ndarray, size: np.ndarray, yaw: float) -> float:
    ...
    l1 = R_rot @ (p1 - center); l2 = R_rot @ (p2 - center)
    min_d = float('inf')
    for alpha in [0, 0.25, 0.5, 0.75, 1.0]:
        p = l1 + alpha * (l2 - l1)
        dx = max(0, abs(p[0]) - size[0]/2); dy = max(0, abs(p[1]) - size[1]/2); dz = max(0, abs(p[2]) - size[2]/2)
...
def clearance(q, obstacles):
    """
    Compute clearance rho(q) for Box, Cylinder, and Sphere obstacles.
    """
    ...
    for obs in obstacles:
        ...
        for k in range(len(pts)-1):
            p1, p2 = pts[k], pts[k+1]
            if obs_type == 'sphere':
                d = _pt_to_segment_dist(p1, p2, center) - dims
            elif obs_type == 'box':
                d = _dist_segment_to_box(p1, p2, center, dims, yaw)
            elif obs_type == 'cylinder':
                d = _dist_segment_to_cylinder(p1, p2, center, dims[0], dims[1])
            else: d = 1.0
            
            if d < local_min: local_min = d
            
        if local_min < min_rho:
            min_rho = local_min
...
set_kinematics(KIN_KR6_R700, JOINT_LIMITS)
...
def plot_bubble_analysis(original_traj, deformed_traj, obstacles,
    ...
    for obs in obstacles:
        ...
        # (Box/Cylinder plotting could be added here for completeness, 
        #  but 3D scatter/wireframe is often sufficient for mission visualization)
