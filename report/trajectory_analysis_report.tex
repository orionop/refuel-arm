...
\vspace{0.2cm}
\textbf{Mathematical Working Example:}

Assume the robot is currently at rest in its HOME pose, where all joints are roughly zero degrees, except the shoulder which hangs down at $-90^\circ$:
\begin{equation*}
    q_{prev} = [0^\circ, -90^\circ, 0^\circ, 0^\circ, 0^\circ, 0^\circ]
\end{equation*}

The robot receives a command to move its End-Effector 5 centimeters forward. The IK-Geo solver calculates 8 possible solutions. After eliminating complex roots and physics limit violations, 2 valid configurations remain:
\begin{align*}
    \text{Solution A (Elbow Up):} \quad & q_A = [2^\circ, -85^\circ, 5^\circ, 0^\circ, 2^\circ, 0^\circ] \\
    \text{Solution B (Elbow Down):} \quad & q_B = [2^\circ, -5^\circ, -95^\circ, 180^\circ, 2^\circ, -180^\circ]
\end{align*}

Our algorithm computes the $\mathcal{L}_2$ Norm (Euclidean distance) from $q_{prev}$ to both $q_A$ and $q_B$:
\begin{align*}
    \text{Dist}_A &= \sqrt{ (2-0)^2 + (-85 - (-90))^2 + (5-0)^2 + (0)^2 + (2-0)^2 + (0)^2 } \\
                  &= \sqrt{ 4 + 25 + 25 + 0 + 4 + 0 } = \sqrt{58} \approx 7.6^\circ \text{ total rotation} \\[10pt]
    \text{Dist}_B &= \sqrt{ (2-0)^2 + (-5 - (-90))^2 + (-95-0)^2 + (180-0)^2 + (2-0)^2 + (-180-0)^2 } \\
                  &= \sqrt{ 4 + 7225 + 9025 + 32400 + 4 + 32400 } = \sqrt{81058} \approx 284.7^\circ \text{ total rotation}
\end{align*}

\textbf{Selection:} The algorithm compares $7.6^\circ < 284.7^\circ$ and automatically selects \textbf{Solution A}. This successfully commands a gentle $7.6^\circ$ cumulative nudge across the motors, explicitly preventing the robot from tearing itself apart in a massive $284^\circ$ swing just to move the tool 5 centimeters.
