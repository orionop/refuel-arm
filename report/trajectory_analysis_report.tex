...
\vspace{0.6cm}
{\Large\textbf{Mathematical Modeling of Obstacle Avoidance Logic}}

The core innovation of the Elastic Strips layer is the use of **Newtonian Force Equilibrium** to refine a pre-planned trajectory in real-time. We treat the path as a deformable string subjected to two competing virtual forces:

\vspace{0.2cm}

\textbf{1. The Internal Contraction Force ($F_{\text{int}}$)}
To keep the trajectory tight and eliminate unnecessary "slack" or movement, we implement a restorative spring force. Mathematically, this is a **Discrete Laplacian Operator** that connects each waypoint to its immediate neighbors:
\begin{equation}
    F_{\text{int}} = k_{c} \cdot (q_{i-1} + q_{i+1} - 2q_{i})
\end{equation}
where $k_{c}$ is the contraction gain and $q_{i}$ represents the joint configuration at waypoint $i$. 
\textit{Logic: If a waypoint deviates from a straight line between its neighbors, this force "pulls" it back toward the local mean, naturally smoothing the trajectory.}

\vspace{0.3cm}

\textbf{2. The External Repulsive Force ($F_{\text{ext}}$)}
To maintain a safe clearance around obstacles (like the car chassis or fuel inlet edges), we implement a **Quadratic Potential Field**. The repulsion force is only active within a defined safety margin $\mathcal{M}$:
\begin{equation}
    F_{\text{ext}} = \begin{cases} k_{r} \cdot \left( \frac{\mathcal{M} - d}{\mathcal{M}} \right)^2 \cdot \hat{n} & \text{if } d < \mathcal{M} \\ 0 & \text{if } d \geq \mathcal{M} \end{cases}
\end{equation}
where $d$ is the Euclidean distance to the obstacle and $\hat{n}$ is the unit vector pointing toward the robot.
\textit{Logic: This creates a virtual "pressure" that grows exponentially as the robot approaches an object, forcing the arm to deflect away from potential collisions.}

\vspace{0.3cm}

\textbf{3. The Force-to-Torque Mapping (Jacobian Transpose $J^T$)}
Because the robot moves in Joint Space ($\theta$) but forces exist in Workspace ($X, Y, Z$), we must bridge the two realms. We use the **Principle of Virtual Work** to map these 3D forces into 6-DOF joint torques ($\tau$):
\begin{equation}
    \tau_{\text{total}} = J^{T} \cdot (F_{\text{int}} + F_{\text{ext}})
\end{equation}
The resulting joint-displacement $\Delta q$ is what the KUKA motors actually execute. 
...
\caption{Force Equilibrium in Elastic Strips: The competition between the restorative "Pull" of internal tension and the proactive "Push" of obstacle repulsion achieves a safe, high-clearance trajectory equilibrium.}
\end{figure}
...
{\Large\textbf{Simplified Logical Overview of Force Dynamics}}

For the purpose of a final mission defense, we can visualize the complex mathematics of the Elastic Strips layer using two simple physical analogies: **The Rubber Band** and **The Magnetic Bumper**.

\vspace{0.6cm}

\textbf{1. The Rubber Band (Internal Force $F_{\text{int}}$)}
Imagine the robot's pre-planned path is a physical rubber band stretched between the start and the goal. 
\begin{itemize}
    \item \textbf{High Tension:} The band naturally wants to be a tight, straight line. Any "slack" or unnecessary curves are pulled inward.
    \item \textbf{Smoothing Effect:} This force ensures that the robot's path is the most efficient possible. It prevents the joints from "wandering" or making large, erratic sweeps that waste energy.
    \item \textbf{Logic:} This is the **"Efficiency Force"** that keeps the movement fluid and professional.
\end{itemize}

\vspace{0.6cm}

\textbf{2. The Magnetic Bumper (External Force $F_{\text{ext}}$)}
Imagine every obstacle (the car door, the fuel port edge) is covered in a powerful magnet that has the same polarity as the robot's arm.
\begin{itemize}
    \item \textbf{Invisible Cushion:} The arm doesn't have to touch the obstacle to feel the effect. There is an invisible "safety cushion" (the 20cm margin $\mathcal{M}$).
    \item \textbf{The Quadratic Effect:} The closer the arm gets, the "harder" the invisible wall becomes. It doesn't just push back gently; it pushes back with **exponentially increasing strength** the deeper you go into the cushion. 
    \item \textbf{Logic:} This is the **"Safety Force"** that guarantees the robot never touches the car, even if the mission path tries to go through it.
\end{itemize}

\vspace{0.6cm}

\textbf{The Natural Equilibrium: The "Sweet Spot"}
The final movement we see in the simulation is the **Equilibrium**. It is the "Sweet Spot" where:
\begin{center}
    \textbf{Internal Pull (Efficiency) = External Push (Safety)}
\end{center}
The robot's arm "bows" outward to dodge the car door because the Magnetic Bumper is pushing it, but it doesn't fly infinitely far away because the Rubber Band is pulling it back toward the goal. This balance ensures the KUKA KR6 is always **Fast, Safe, and Precise**.
