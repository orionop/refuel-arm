...
\begin{itemize}
    ...
    \item \textbf{Every Single Waypoint In Between (The Cartesian Phase):} This is the really cool part. To guarantee the arm slides in a perfectly straight 8cm line, we generate 20 microscopic "waypoints" in 3D space between the Pre-Approach coordinates and Target coordinates. IK-Geo runs 20 separate times—once for every single millimeter of movement—to calculate the exact joint angles required to keep the nozzle perfectly straight during insertion.
\end{itemize}

\textit{(In the older version of the code that failed, IK-Geo only calculated the Target pose, and we just guessed the rest in C-Space, which caused the collision/instability issues when squeezing past obstacles!)}
...
\begin{enumerate}
    ...
    \item \textbf{The Shortest Path Selector ($\mathcal{L}_2$ Norm):} We calculate the Euclidean Distance ($\mathcal{L}_2$ Norm) between the robot's current position ($q_{prev}$) and all remaining valid solutions in 6D joint-space. The algorithm simply sorts the valid roots and always selects the one with the absolute lowest $\mathcal{L}_2$ distance. This guarantees smooth, continuous tracking and prevents the robot from violently swinging its joints in a massive circle just to move the tool a few centimeters.
\end{enumerate}
...
The robot receives a command to move exactly to the Target Pose from Section 6: \textbf{coordinates $[0.706, 0.000, 0.413]$ (m) with a pitch angle of $15^\circ$}. The IK-Geo solver calculates 8 possible solutions. After applying the KUKA physical joint limits, exactly 4 valid physical configurations remain:
\begin{align*}
    \text{Solution A:} \quad & q_A = [\phantom{18}0.0^\circ, -36.97^\circ, \phantom{-}67.74^\circ, \phantom{-18}0.0^\circ, -15.76^\circ, \phantom{-18}0.0^\circ] \\
    \text{Solution B:} \quad & q_B = [\phantom{18}0.0^\circ, \phantom{-}29.98^\circ, -59.90^\circ, \phantom{-18}0.0^\circ, \phantom{-}44.92^\circ, \phantom{-18}0.0^\circ] \\
    \text{Solution C:} \quad & q_C = [\phantom{18}0.0^\circ, -36.97^\circ, \phantom{-}67.74^\circ, -180.0^\circ, \phantom{-}15.76^\circ, \phantom{-}180.0^\circ] \\
    \text{Solution D:} \quad & q_D = [\phantom{18}0.0^\circ, \phantom{-}29.98^\circ, -59.90^\circ, \phantom{-}180.0^\circ, -44.92^\circ, -180.0^\circ]
\end{align*}

Our algorithm computes the $\mathcal{L}_2$ Norm (Euclidean distance) from $q_{prev}$ to all 4 solutions. For brevity, let's look at the distance to Solution A (the safest geometry) and Solution D (the most twisted geometry):
\begin{align*}
    \text{Dist}_A &= \sqrt{ (0)^2 + (-36.97 - (-90))^2 + (67.74)^2 + (0)^2 + (-15.76)^2 + (0)^2 } \\
                  &= \sqrt{ 0 + 2812.2 + 4588.7 + 0 + 248.4 + 0 } = \sqrt{7649.3} \approx 87.45^\circ \text{ total rotation} \\[10pt]
    \text{Dist}_D &= \sqrt{ (0)^2 + (29.98 - (-90))^2 + (-59.90)^2 + (180)^2 + (-44.92)^2 + (-180)^2 } \\
                  &= \sqrt{ 0 + 14395.2 + 3588.0 + 32400.0 + 2017.8 + 32400.0 } = \sqrt{84801.0} \approx 291.21^\circ \text{ total rotation}
\end{align*}

\textbf{Selection:} $87.45^\circ < 141.43^\circ < 269.16^\circ < 291.21^\circ$. The algorithm automatically selects \textbf{Solution A}. This successfully commands a gentle $87.45^\circ$ cumulative nudge across the motors (mostly raising the shoulder and elbow), explicitly preventing the robot from twisting its wrists up to $360^\circ$ unnecessarily (as would happen in Solution C or D).
